...
export async function saveCertificate(formData: FormData) {
  ...
  if (id) await db.certificate.update({ where: { id }, data });
  else await db.certificate.create({ data });
...
export async function submitReturn(formData: FormData) {
  ...
  await db.quarterlyReturn.upsert({
    ...
    create: { period, tonnage, submittedAt: new Date(), preparedBy: user.name, reference: String(formData.get('reference') ?? '') },
  });
