...
export async function GET(request: Request) {
  ...
  if (table === 'batches') {
    const data = await db.batch.findMany({ where: { company: { in: companies } }, include: { product: true, supplier: true }, orderBy: { receivedAt: 'desc' } });
    rows = data.map((b) => ({
    ...
    const data = await db.stockMovement.findMany({ where: { product: { company: { in: companies } } }, include: { product: true, batch: true, user: true }, orderBy: { at: 'desc' } });
    rows = data.map((m) => ({
    ...
    const data = await db.ncr.findMany({ where: { company: { in: companies } }, include: { order: true, customer: true, supplier: true, batch: true, raisedBy: true }, orderBy: { raisedAt: 'desc' } });
    rows = data.map((n) => ({
    ...
    const data = await db.customer.findMany({ where: { company: { in: companies } }, include: { accountManager: true }, orderBy: { name: 'asc' } });
    rows = data.map((c) => ({
