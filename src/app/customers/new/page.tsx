...
export default async function NewCustomerPage() {
  ...
  const [alerts, managers, towns, codes] = await Promise.all([
    getAlerts(user),
    ...
    codeOptions(getActiveCompany(user)),
  ]);
  ...
  return (
    <Shell user={user} module="customers" nav={NAV.customers} current="/customers" alerts={alerts.length}>
      ...
      <CustomerForm user={user} action={createCustomer} managers={managers} towns={towns.map((t) => t.name)} codes={codes} submitLabel="Open the account" />
    </Shell>
  );
}
